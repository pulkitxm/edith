...
Pushing the release commit needs a `RELEASE_PUSH_TOKEN` secret and pushing to the
tap needs a `TAP_PUSH_TOKEN` secret: fine grained personal access tokens scoped to
`pulkitxm/edith` and `pulkitxm/homebrew-tap` with read and write access to
contents. Without them the release is blocked before a version is cut.
