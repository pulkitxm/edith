...
exposed by a machine. The app's Control Center separately shows every readable
hwmon fan in RPM alongside the active profile and the choices the kernel reports.
...
omit the matching part of Control Center.
...
Reading profile state needs no privilege on a normal Linux installation. Control
Center's separate hardware fan readings are unprivileged too.
