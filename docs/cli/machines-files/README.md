...
the `$HOME` probe, 20 for the file-kind probe `put` makes, 30 for the `stat`
check after an upload, 45 for a directory listing or final staged publication,
120 for `search` and `info`, 300 for `cp`, `mv`, `rename`, `mkdir`, `rm` and
`duplicate`, and 20 for the `undo` reply. The byte transfer itself has no total
duration cap, because a slow file is not necessarily a broken one.

A failed command and a connection that cannot be opened are different exit
codes. A command the machine ran and rejected exits 1 with the machine's own
message appended, while a connection that cannot be established exits 4. Once
a multi-file transfer begins, individual failures are collected into its one
outcome and any failed item makes the final status 1.
...
document has a different shape from its success. Transfer progress on
`get-many` and `transfer` is suppressed by `--json`, because it is stderr
furniture rather than stdout data.

`--yes` confirms `--replace` for `get`, `get-many`, `put`, `transfer`, `cp` and
`mv`. It also confirms `rm --delete`. Passing it without the matching mode
changes nothing.

The Files window and the CLI share the same transfer, `search`, `info`,
`duplicate`, `undo`, `rename` and `rm` operation contracts. Their paths,
timeouts and safety rules therefore stay aligned. Permanent removal previews
without connecting and does not run until `--yes` is present. The window
presents the equivalent confirmation before it asks the same operation executor
