...
Prints events in a bounded date window shared with the Calendar UI.
...
| --- | --- | --- | --- |
| `--days <n>` | integer from 0 through 120 | `7` | Read from midnight today through this many days. Zero ends at the moment the command starts. |
...
Nothing is written. `ls` sends a typed date window to the app, which reads
EventKit for that request. The UI and CLI share the same 120-day bound, sorting
and deduplication. A CLI read does not widen or replace the UI's current page.
...
trip. You have to write `--days=-1` to reach the negative-value check because
`--days -1` is parsed as a missing value.
...
Values above the shared maximum also exit 2:

```text
$ ed calendar ls --days 121
error: --days must be between 0 and 120
```
