...
in Finder; a missing one opens its containing data folder. The iCloud directory
is created before opening when needed, matching the settings UI.

Plain output names the action and exact path. JSON has `id`, `url`, `mode`, and
`opened`. `mode` is `open` or `reveal`. A parser error exits 2, and a macOS open
failure exits 4. The command needs no Edith process.
