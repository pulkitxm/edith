...
The machine argument accepts the same exact name, SSH alias, UUID or unambiguous
prefix as other machine commands. `ed` resolves it to the stable machine UUID
before contacting the app. The app uses that UUID to select terminal tab sets,
so renaming a machine cannot send the line to a different machine.
...
time. It exits 3 when the machine is unknown or has no open terminal tabs.
