            ...
            VStack(alignment: .leading, spacing: UIScale.pt(8)) {
                if let question = model.question {
                    ...
                    HStack(spacing: UIScale.pt(8)) {
                        deskButton("Answer", filled: true) { await model.answer() }
                        deskButton("Not now") { await model.skip() }
                        deskButton("Never ask about \(question.topic)") { await model.mute() }
                    }
                } else if let resolution = model.lastResolution {
                    Text(resolution)
                        .font(.system(size: UIScale.pt(13)))
                        .foregroundStyle(DashSkin.ink(dark))
                    deskButton("Anything else?") { await model.askNext() }
                } else {
                    Text(
                    ...
                    deskButton("What do you want to know?") { await model.askNext() }
                }
