        ...
        SkinCard(title: "Add a machine", note: "the stack can run anywhere", dark: dark) {
            VStack(alignment: .leading, spacing: UIScale.pt(8)) {
                HStack(spacing: UIScale.pt(8)) {
                    ...
                    CompanionButton(
                        title: "Add", role: .primary, busy: model.busy, disabled: model.busy
                    ) {
                            ...
                            HStack(alignment: .firstTextBaseline, spacing: UIScale.pt(8)) {
                                ...
                                MindChip(
                                    ...
                                    tone: machine.effectiveProfile == "cpu-only"
                                        ? DashSkin.gold : DashSkin.sage)
                                Spacer(minLength: 0)
                                CompanionButton(
                                    title: "Probe", busy: model.busy, disabled: model.busy
                                ) {
                                    Task { await model.probe(machine) }
                                }
                            }
