...
@Suite @MainActor struct KeyboardToolsRenderTests {
    @Test func settingsRenderWithIsolatedPreferences() throws {
        _ = TestWindowHost.application
        let defaults = SharedDefaults.store
        let key = AppStorageKeys.KeyboardTools.enabled
        let previous = defaults.object(forKey: key)
        defaults.set(true, forKey: key)
        defer { defaults.set(previous, forKey: key) }
        let view = Form { KeyboardToolsRows() }
            .formStyle(.grouped)
        ...
        if let directory = ProcessInfo.processInfo.environment["EDITH_RENDER_DUMP"] {
            try data.write(
                to: URL(fileURLWithPath: directory).appendingPathComponent("keyboard-tools.png"))
        }
    }
}
