...
@Suite struct AgentOperationCatalogTests {
    ...
    @Test func theServedListIncludesAgentControlAndUsageRefreshOperations() {
        ...
        #expect(AgentOperationCatalog.served == control + usage + NetworkDiagnosticOperation.allCases.map { $0.descriptor.id })
    ...
    @Test func everyServedOperationIsInTheUserOperationCatalog() {
        ...
        for descriptor in AgentOperationCatalog.descriptors {
            #expect(descriptor.cli.first == "agent" || descriptor.cli.first == "usage" || descriptor.cli.first == "network")
        }
    }
