...
@Suite @MainActor struct FinderToolsRenderTests {
    @Test func settingsRenderWithEnabledActions() throws {
        ...
        if let directory = ProcessInfo.processInfo.environment["EDITH_RENDER_DUMP"] {
            ...
            try data.write(to: URL(fileURLWithPath: directory).appendingPathComponent("finder-tools.png"))
        }
    }
}
