...
public struct DOCXWriter {
    ...
    public static func write(
        ...
        for block in blocks {
            ...
            case let .table(rows):
                body += table(rows, width: usableWidth)
            case let .image(data, ext, width, height):
                imageCount += 1
                let id = "rIdImage\(imageCount)"
                let safe = OOXMLPackage.imageExtension(ext)
                extensions.insert(safe)
                let name = "image\(imageCount).\(safe)"
        ...
        let orientation = pageSize.width > pageSize.height ? #" w:orient="landscape""# : ""
        let document =
            header
            + #"<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture"><w:body>"#
            + body
            + #"<w:sectPr><w:pgSz w:w="\#(twips(pageSize.width))" w:h="\#(twips(pageSize.height))"\#(orientation)/><w:pgMar w:top="\#(twips(margin))" w:right="\#(twips(margin))" w:bottom="\#(twips(margin))" w:left="\#(twips(margin))" w:header="720" w:footer="720" w:gutter="0"/></w:sectPr>"#
            + "</w:body></w:document>"
        package.add(
            "[Content_Types].xml",
            header
    ...
    static func paragraph(_ runs: [Run], heading: Int?) -> String {
        var xml = "<w:p>"
        if let heading {
            xml += #"<w:pPr><w:pStyle w:val="Heading\#(min(max(heading, 1), 6))"/></w:pPr>"#
        }
        for run in runs {
            xml += "<w:r><w:rPr>"
            ...
            if let color = run.color.flatMap(StudioColor.init(hex:)) {
                xml += #"<w:color w:val="\#(color.hex.dropFirst().prefix(6))"/>"#
            }
            let half = min(3276, max(2, Int((run.size * 2).rounded())))
            xml += #"<w:sz w:val="\#(half)"/><w:szCs w:val="\#(half)"/></w:rPr>"#
            xml += content(run.text)
            xml += "</w:r>"
        }
        return xml + "</w:p>"
    }
    ...
    static func content(_ text: String) -> String {
        ...
        for (index, line) in normalized.components(separatedBy: "\n").enumerated() {
            if index > 0 { xml += "<w:br/>" }
            for (part, piece) in line.components(separatedBy: "\t").enumerated() {
                if part > 0 { xml += "<w:tab/>" }
                if !piece.isEmpty {
                    xml += #"<w:t xml:space="preserve">\#(OOXMLPackage.escape(piece))</w:t>"#
                }
            }
        }
        return xml
    ...
    static func table(_ rows: [[String]], width: Double = 504) -> String {
        let columns = rows.map(\.count).max() ?? 0
        guard columns > 0 else { return "" }
        let cellWidth = max(1, Int((width * 20).rounded()) / columns)
        var xml =
            ...
            + String(repeating: #"<w:gridCol w:w="\#(cellWidth)"/>"#, count: columns)
            + "</w:tblGrid>"
        for row in rows {
            xml += "<w:tr>"
            for column in 0..<columns {
                ...
                xml +=
                    #"<w:tc><w:tcPr><w:tcW w:w="\#(cellWidth)" w:type="dxa"/></w:tcPr><w:p><w:r>\#(content(value))</w:r></w:p></w:tc>"#
            }
            xml += "</w:tr>"
        }
    ...
    static var styles: String {
        let headings = (1...6).map { level -> String in
            let size = [0, 40, 32, 26, 24, 22, 22][level]
            return
