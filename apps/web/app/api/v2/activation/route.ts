...
import { activationV2BodySchema } from "@/lib/validation";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const route = "/api/v2/activation";
...
export async function POST(request: Request): Promise<Response> {
  ...
  const parsed = activationV2BodySchema.safeParse(await readJsonBody(request));
  ...
  try {
    ...
    result = await activateDeviceV2(licenseStore, parsed.data);
  } catch {
